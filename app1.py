...
if uploaded_file:
    ...
    # Section 1 – Daily Energy Usage Chart
    st.subheader("📊 Daily Energy Usage (kW)")
    st.line_chart(daily_df['global_active_power'])

    # Section 2 – Anomalies Table
    st.subheader("🚨 Detected Anomalies")
    st.dataframe(daily_df[daily_df['anomaly'] == -1][['global_active_power', 'voltage', 'anomaly_label']])

    # Section 3 – SmartWatt Recommendations
    st.subheader("💡 SmartWatt Recommendations")

    # View filter options
    filter_option = st.radio(
        "Choose recommendation view:",
        ["Show All", "Only Anomalies", "Export as CSV"],
        horizontal=True
    )

    if filter_option == "Show All":
        st.dataframe(daily_df[['global_active_power', 'voltage', 'anomaly_label', 'recommendations']])
    elif filter_option == "Only Anomalies":
        st.dataframe(daily_df[daily_df['anomaly'] == -1][['global_active_power', 'voltage', 'anomaly_label', 'recommendations']])
    elif filter_option == "Export as CSV":
        csv = daily_df[['global_active_power', 'voltage', 'anomaly_label', 'recommendations']].to_csv(index=True).encode('utf-8')
        st.download_button(
            label="📥 Download Recommendations as CSV",
            data=csv,
            file_name="smartwatt_recommendations.csv",
            mime='text/csv'
        )
